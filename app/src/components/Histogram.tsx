      ...
      <div className="relative">
        ...
        >
          ...
          {bins.map((bin, i) => {
            ...
            return (
              <rect key={i} x={x} y={y} width={bw} height={barH} rx="2" fill={bin.isAthlete ? color : "#374151"} />
            );
          })}
          ...
          {athleteBinIdx >= 0 && (
            <line
              x1={MARGIN.left + ((athleteBinIdx + 0.42) / bins.length) * plotWidth}
              y1={MARGIN.top}
              x2={MARGIN.left + ((athleteBinIdx + 0.42) / bins.length) * plotWidth}
