...
- **Athlete profiles** (`data/athlete-profiles.json`) is gitignored; compact slug→race mapping generated at build time
...
- `scripts/build-search-index.js` — Pre-builds deduplicated athlete search index from CSVs (runs automatically during `npm run build`)
